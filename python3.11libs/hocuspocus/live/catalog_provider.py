...
def _parameters(node_type: Any) -> tuple[ParameterDefinition, ...]:
    ...
    def visit(items: Iterable[Any]) -> None:
        for item in items:
            ...
            if isinstance(children, (tuple, list)):
                if _template_type(item).lower() == "multiparmblock":
                    parameter = _parameter(item, operator_name)
    ...
    if isinstance(sections, Mapping):
        for name, section in sorted(sections.items(), key=lambda item: str(item[0])):
            ...
            if isinstance(content, str):
                has_surrogates = any(0xD800 <= ord(character) <= 0xDFFF for character in content)
                content = content.encode("utf-8", errors="surrogatepass") if not has_surrogates else b""
            if not isinstance(content, bytes):
                content = b""
            # Embedded Contents sections can contain session-specific compiled
            # bytes. Their stable size still detects structural drift; textual
            # sections retain a full content digest.
            stable_content = b"" if str(name).casefold().startswith("contents") or not content else content
...
class LiveHoudiniCatalogProvider:
    ...
    def _package_dirs(self) -> tuple[Path, ...]:
        ...
        candidates = split_paths(environment("HOUDINI_PACKAGE_DIR"))
        ...
        if hsite:
            site = Path(hsite)
            candidates.append(site / "packages")
            if isinstance(version_tuple, (tuple, list)) and len(version_tuple) >= 2:
    ...
    def build_catalog(self) -> CatalogSnapshot:
        ...
        for category_name, node_type in pending:
            ...
            operators[(category_name, raw_name)] = OperatorDefinition(
                ...
                source=source,
                parameters=_parameters(node_type),
                inputs=_connectors(node_type, category_name, "Input"),
                outputs=_connectors(node_type, category_name, "Output"),
                spare_parameter_policy="declared_only" if definition is not None else "forbidden",
