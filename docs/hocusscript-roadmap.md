...
The historical safe `0.2` slice was same-project static imports with typed scalar/`node_output` parameters and exports; it is complete through G5 and remains frozen. The current safe H1 slice adds strict, observational `0.3` carrier/schema decoding to H0 parsing and canonical formatting. It excludes semantic expansion, compiler/resolver/lock-writer/CLI/editor enablement, live schema registration, MCP file access, document/live consumption, and recursion until their named gates close.
