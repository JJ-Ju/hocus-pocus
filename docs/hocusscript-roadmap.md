...
Typed-control native integration status (Batch H3A, 2026-07-25): in progress. The explicit same-project lane now accepts only a fully pinned schema-v4/language-`0.3` project and canonical project-relative entry, rejects external aliases, performs bounded stable reads, resolves the complete verified local module closure, and emits strict resolved-module-set v2 with final project/lock/catalog/source/winner rechecks. Before selected expansion it validates every authored operator and parameter surface in the entry and complete module closure against the pinned immutable catalog, including unselected branches and zero-count fold bodies. The one-shot native compiler then emits GraphSpec `0.4`, runs selected graph semantics, unions hidden-body capabilities, and self-authenticates deterministic Bundle `0.4`. Manifest-selected native `format`, `check`, and `compile` are enabled for this lane; the real CLI smoke produced compiler `0.5.0` / Bundle `0.4` and rejected an invalid hidden-branch operator. H3 remains open for resolver-derived v4 lock publication, explicit external module-manifest-v2 roots, and control-aware project editor completion/navigation. Document/live lowering and every MCP project/file/root surface remain closed.
...
8. deliver proposed language `0.3` control through separately reviewable H sub-batches:
   ...
   - H3 native integration: enable verified project resolver/compiler/lock, CLI check/format/compile, and project-aware editor completion/navigation only through the new version lane; retain explicit user-selected project/module roots and the content-only MCP boundary; drive batches with Ruff, focused regressions, and a real user-facing workflow smoke
