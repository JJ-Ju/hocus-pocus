...
7. enable external aliases only through staged gates: G1 explicit root/manifest inspection, G2 read-only complete mixed-root lock planning, G3 leased atomic external-record publication, and G4 separate exact-root compiler/editor/bundle consumption are complete; G5 explicit CLI `--module-root alias=absolute-path` wiring remains pending, while MCP remains content-only
