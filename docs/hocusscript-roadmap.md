...
7. enable external aliases only through staged gates: G1 explicit root/manifest inspection and G2 read-only complete mixed-root lock planning are complete; G3 leased atomic external-record publication, G4 shared compiler/editor resolution, and G5 explicit CLI `--module-root alias=absolute-path` wiring remain pending, while MCP remains content-only
