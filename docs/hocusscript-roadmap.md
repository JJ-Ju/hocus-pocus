...
7. enable external aliases only through staged gates: G1 explicit root/manifest inspection, G2 read-only complete mixed-root lock planning, and G3 leased atomic external-record publication are complete; G4 shared compiler/editor/bundle resolution and G5 explicit CLI `--module-root alias=absolute-path` wiring remain pending, while MCP remains content-only
